...
public class UIControl extends Application { // ส่วนแสดงผล UI
    ...
    private final Map<Province, ImageView> nodeSprites = new HashMap<>();
    private final Map<String, Line> edgeLines = new HashMap<>();
    private final Image[] TruckImages = new Image[7];
    ...
    public void moneyPopup(int change) {
        ...
        StackPane.setMargin(moneypopup, new javafx.geometry.Insets(25, 0, 0, 20));
    ...
    public void highlightTruckRoute() {
        ...
        Province currentLoc = parcels.get(0).getFrom();
    ...
    public java.util.List<Parcel> getParcelsInTruck() {
        ...
        for (int i = 4; i >= 0; i--) { // เรียงใหม่ให้กลับด้าน
            ...
            if (!slot.getChildren().isEmpty()) {
                ...
                if (parcelNode.getUserData() instanceof Parcel p) {
                    parcels.add(p);
                }
            }
        }
    ...
    public void showDailySummary(int parcelDelivered, int income, double distance, int expenses) {
        ...
        valDistance.setStyle("-fx-text-fill: #facc15; -fx-font-size: 18px; -fx-font-weight: bold;");

        Label lblIncome = new Label("Gross Revenue:");
        lblIncome.setStyle(labelStyle);
        Label valIncome = new Label("+ ฿ " + String.format("%,d", income));
        valIncome.setStyle("-fx-text-fill: #4ade80; -fx-font-size: 18px; -fx-font-weight: bold;");
        ...
        valQuota.setStyle("-fx-text-fill: #4ade80; -fx-font-size: 18px; -fx-font-weight: bold;");
        ...
        valExpenses.setStyle("-fx-text-fill: #ef4444; -fx-font-size: 18px; -fx-font-weight: bold;");

        statsGrid.add(lblDelivered, 0, 0);
        statsGrid.add(valDelivered, 1, 0);
        statsGrid.add(lblDistance, 0, 1);
        statsGrid.add(valDistance, 1, 1);
        statsGrid.add(lblIncome, 0, 2);
        statsGrid.add(valIncome, 1, 2);
        statsGrid.add(lblQuota, 0, 3);
        statsGrid.add(valQuota, 1, 3);
        statsGrid.add(lblExpenses, 0, 4);
        statsGrid.add(valExpenses, 1, 4);
